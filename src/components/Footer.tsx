...
function PageNavigation() {
  ...
  const isJa = currentLocale === 'ja-JP'
  const prevLabel = isZh ? '上一页' : isJa ? '前へ' : 'Previous'
  const nextLabel = isZh ? '下一页' : isJa ? '次へ' : 'Next'

  return (
    <div className="flex">
      {previousPage && (
        <div className="flex flex-col items-start gap-3">
          <PageLink label={prevLabel} page={previousPage} previous />
        </div>
      )}
      {nextPage && (
        <div className="ml-auto flex flex-col items-end gap-3">
          <PageLink label={nextLabel} page={nextPage} />
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="flex flex-col items-center justify-between gap-5 border-t border-zinc-900/5 pt-8 sm:flex-row dark:border-white/5">
      ...
      <div className="flex gap-4">
        <SocialLink href="https://x.com/PopLabAI" icon={XIcon}>
          Follow us on X
        </SocialLink>
        <SocialLink href="https://github.com/PopLabAI" icon={GitHubIcon}>
          Follow us on GitHub
        </SocialLink>
